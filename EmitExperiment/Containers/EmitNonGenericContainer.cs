...
namespace EmitExperiment.Containers
{
    ...
    /// </summary>
    public class EmitNonGenericContainer: IContainer
    {
        #region Constructor

        public EmitNonGenericContainer()
        {
        ...
        private Func<IContainer, object> CreateFactory(Type t)
        {
            ...
            var containerGet = containerType.GetMethod(nameof(Get), new [] { typeof(Type) } );
            var typeofGet = typeof(Type).GetMethod(nameof(Type.GetTypeFromHandle));
            ...
            foreach (var arg in args)
            {
                ...
                il.Emit(OpCodes.Ldtoken, arg.ParameterType);
                il.Emit(OpCodes.Call, typeofGet);
                il.Emit(OpCodes.Callvirt, containerGet);
            }
